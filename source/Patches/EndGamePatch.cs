        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)] EndGameResult endGameResult)
        {
            ...
            foreach (var playerControl in PlayerControl.AllPlayerControls)
            {
                ...
                foreach (var role in Role.RoleHistory.Where(x => x.Key == playerControl.PlayerId))
                {
                    ...
                    else if (role.Value == RoleEnum.Miner) { playerRole += "<color=#" + Patches.Colors.Impostor.ToHtmlStringRGBA() + (GameOptionsManager.Instance.currentNormalGameOptions.MapId == 5 ? ">Mycologist</color> > " : ">Miner</color> > "); }
                    ...
                    else if (role.Value == RoleEnum.Sage) { playerRole += "<color=#" + Patches.Colors.Sage.ToHtmlStringRGBA() + ">Sage</color> > "; }
                    else if (role.Value == RoleEnum.Demagogue) { playerRole += "<color=#" + Patches.Colors.Impostor.ToHtmlStringRGBA() + ">Demagogue</color> > "; }
