    ...
    public class AddShootButton
    {
        ...
        private static Action Shoot(Deputy role, byte targetId)
        {
            void Listener()
            {
                ...
                NotificationPatch.Notification(Patches.TranslationPatches.CurrentLanguage == 0 ? $"Deputy Has Shot {target.GetDefaultOutfit().PlayerName}!" : $"Deputy Strzelil {target.GetDefaultOutfit().PlayerName}", 1000 * CustomGameOptions.NotificationDuration);
                role.Revealed = true;
