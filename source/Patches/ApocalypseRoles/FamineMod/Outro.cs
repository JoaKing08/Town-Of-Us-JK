...
namespace TownOfUs.ApocalypseRoles.FamineMod
{
    ...
    public static class Outro
    {
        public static void Postfix(EndGameManager __instance)
        {
            ...
            (x.Is(RoleEnum.Plaguebearer) || x.Is(RoleEnum.Pestilence))) > 0) return;
            var role = Role.AllRoles.FirstOrDefault(x =>
            x.RoleType == RoleEnum.Famine && ((Famine)x).ApocalypseWins && !x.Player.Data.IsDead && !x.Player.Data.Disconnected);
            if (role == null) return;
