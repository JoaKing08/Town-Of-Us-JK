...
namespace TownOfUs
{
    ...
    public class OpenDoorConsoleCanUse
    {
            ...
            ref bool __state)
        {
            ...
            if (((playerControl.Is(RoleEnum.Phantom) && !Role.GetRole<Phantom>(playerControl).Caught) || (playerControl.Is(RoleEnum.Haunter) && !Role.GetRole<Haunter>(playerControl).Caught) || (playerControl.Is(RoleEnum.Poltergeist) && !Role.GetRole<Poltergeist>(playerControl).Caught) || (playerControl.Is(RoleEnum.Harbinger) && !Role.GetRole<Harbinger>(playerControl).Caught)) && playerInfo.IsDead)
            {
