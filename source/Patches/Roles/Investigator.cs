            ...
            if (matches.Count() > 0)
            {
                ...
                switch (reportType)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        if (player.Is(Faction.Impostors)) message += Patches.TranslationPatches.CurrentLanguage == 0 ? "'s body had a knife next to it <b>(Player Was <color=#FF0000FF>Impostor</color>)</b>" : $" <b>{player.GetDefaultOutfit().PlayerName}</b> mial nóz przy sobie <b>(Graczem Byl <color=#FF0000FF>Impostor</color>)</b>";
                        ...
                        else if (player.Is(Faction.Crewmates)) message += Patches.TranslationPatches.CurrentLanguage == 0 ? " was just doing their tasks <b>(Player Was <color=#00FFFFFF>Crewmate</color>)</b>" : $" <b>{player.GetDefaultOutfit().PlayerName}</b> tylko robil zadania <b>(Graczem Byl <color=#00FFFFFF>Crewmate</color>)</b>";
                        break;
                    case 2:
                    ...
                    case 6:
                        switch (killerRole.Faction)
                        {
                            ...
                            case Faction.Impostors:
                                message += Patches.TranslationPatches.CurrentLanguage == 0 ? "'s body was covered in stab wounds <b>(Killed By <color=#FF0000FF>Impostor</color>)</b>" : $" <b>{player.GetDefaultOutfit().PlayerName}</b> byl pokryty w ranach cietych <b>(Zabity Przez <color=#FF0000FF>Impostor</color>)</b>";
                                break;
                        }
                        break;
                }
