...
namespace TownOfUs.Roles
{
    public class Glitch : Role, IVisualAlteration
    {
        ...
        public void Update(HudManager __instance)
        {
            ...
            {
                Utils.SetClosestPlayer(ref ClosestPlayer, float.NaN, PlayerControl.AllPlayerControls.ToArray().Where(x => !(FactionOverride == FactionOverride.Undead && x.Is(FactionOverride.Undead)) && !(FactionOverride == FactionOverride.Recruit && x.Is(FactionOverride.Recruit) && !(x.Is(RoleEnum.Jackal) && !CustomGameOptions.RecruistSeeJackal))).ToList());
            }
