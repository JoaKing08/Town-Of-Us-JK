    ...
    public class AddShootButton
    {
        ...
        private static Action MeetingKill(Demagogue role, byte targetId)
        {
            void Listener()
            {
                ...
                NotificationPatch.Notification(Patches.TranslationPatches.CurrentLanguage == 0 ? $"Demagogue Has Convicted {target.GetDefaultOutfit().PlayerName}!" : $"Demagogue Skazal {target.GetDefaultOutfit().PlayerName}", 1000 * CustomGameOptions.NotificationDuration);
